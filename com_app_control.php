...
<?php
  if(isset($_GET['msg']))
     ...
     <strong>Αίτηση Εγκρίθηκε!</strong> Ο φοιτητής θα ενημερωθεί.
     <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
     </div> ' ;

  if(isset($_GET['msg1']))
     echo '<div class="alert alert-warning alert-dismissible fade show" role="alert">
     <strong>Αίτηση Απορρίφθηκε!</strong> Ο φοιτητής θα ενημερωθεί.
     <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
     </div> '   ;
  ?>
  ...
  <div class="container">
    <div class="grid_12">
      <h3>Παρακολούθηση Αιτήσεων για τις Θέσεις μου</h3>
    </div>
  ...
  <tbody class="table-group-divider" method="post">
     <?php
        ...
        while ($row = mysqli_fetch_assoc($result)){
          if ($_GET['id']==$row['company_name']){
            ...
            ?>
            <tr>
             <td><a scope="row" href="application.php?id=<?php echo $row['application_id'] ?>" class="link"><?php echo $row['application_id'] ?></a></td>
             <td ><?php echo $row['position_name'] ?></td>
             <td ><?php echo $row1['NAME'] ?> <?php echo $row1['SURNAME'] ?></td>
             <td><?php echo $row['stage'] ?></td>
             <th>
               <?php
               if($row['stage']=='Εκκρεμής'){
                $enable = true;//vale ta buttons kanonika 
               }
               else{
                $enable=false;//vale ta disabled buttons
               }
               ?> 
               <a href="approved.php?id=<?php echo $row['application_id'] ?>&com=<?php echo $row['company_name'] ?>" name="approve" class="btn btn-success"><i class="fa-solid fa-user-check"></i> Έγγριση</a>
               <a href="rejected.php?id=<?php echo $row['application_id'] ?>" class="btn btn-danger"><i class="fa-solid fa-user-xmark"></i> Απόρριψη</a>
             </th>
            </tr>
            <?php
        }
